...
test.beforeEach(async ({ page }) => {
  ...
  await page.waitForSelector('#historias-table tbody', { state: 'attached' });
});

test('ver lista inicial', async ({ page }) => {
  await expect(page.locator('#historias-table tbody')).toBeEmpty();
...
test('crear historia con diagnostico HTML', async ({ page }) => {
  ...
  await expect(page.locator('#modal-historia')).toBeVisible();
